...
//   - the README at GitHub's content width, from the @2x image
...
import { PREVIEW_DIR, previewProblems, RETINA, SOCIAL } from "./image-facts.mjs";
...
async function views(imageDir) {
	...
	try {
		...
		await readme.setContent(`<body style="margin:0"><img src="${dataUrl(join(imageDir, RETINA.file))}" style="width:${README_WIDTH}px;display:block">`);
...
export async function runChecks({ imageDir = join(root, PREVIEW_DIR), strict = true, open = false } = {}) {
	const problems = strict ? previewProblems(root) : [];
